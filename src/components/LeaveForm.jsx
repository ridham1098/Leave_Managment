...
function LeaveForm({ onSubmit, MAX_LEAVES, leaves }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (docRef?.id) {
        ...
        if (result.success) {
          toast.success(
            "Leave has been Submitted successfully ✉️"
          );
        } else {
          toast.success(
            "Leave submitted! Email notification failed."
          );
        }
      } else {
  ...
  return (
    <form
      ...
      className="my-8 p-6 border border-white/20 rounded-2xl bg-white/10 backdrop-blur-lg shadow-2xl text-white space-y-6 transition-all duration-300 hover:shadow-blue-400/40 w-full max-w-lg mx-auto hover:scale-105"
    >
      ...
      <p className="text-sm text-gray-300 text-center mb-4">
        <span className="font-bold">Note:</span> For a single-day
        leave, please enter the same date in both{" "}
        <span className="font-bold">From Date</span> and{" "}
        <span className="font-bold">To Date</span> fields.
      </p>
    </form>
  );
}
